...
@ScriptManifest(
        name = "dTempRestoreMaker",
        description = "Makes super restores in any of the dynamic banks.",
        version = "1.0",
        guideLink = "",
...
public class dTempRestoreMaker extends AbstractScript {
    ...
    int processCount = 0;
    int productIndex;
    int[] reqItems = new int[]{ItemList.SNAPDRAGON_POTION_UNF_3004, ItemList.RED_SPIDERS_EGGS_223};
    ...
    public void onStart(){
        ...
        Logger.log("Thank you for using the dTempRestoreMaker script!");
        ...
        productIndex = Paint.createBox("Super restore", ItemList.SUPER_RESTORE4_3024, processCount);
    ...
    private void executeMakePotMethod() {
        ...
        if (!Inventory.contains(ItemList.SNAPDRAGON_POTION_UNF_3004, 0.75) && !Inventory.contains(ItemList.RED_SPIDERS_EGGS_223, 0.75)) {
            Logger.log("We don't have a UNFINISHED POTIONS and SPIDER EGGS in our inventory, going back to banking!");
            return;
        }
        ...
        while (Inventory.containsAll(reqItems, 0.8)) {
            readXP();
    ...
    private void checkInventPotMethod() {
        ...
        if (!Inventory.containsAll(reqItems, 0.75)) {
